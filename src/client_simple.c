...
int telecharger(Client *client, int fichier_dl)
{
    ...
    int connection = connect(client->tracker.socket_client, (struct sockaddr *)client->tracker.socket_serveur, sizeof(struct sockaddr_in));
    if(connection < 0)
    {
        perror("connect");
        exit(EXIT_FAILURE);
    ...
    client->fd_local = open("fichier", O_TRUNC|O_WRONLY|O_CREAT, 0777);
...
void client(void *donnees)
{
    ...
    donnes->tracker.meilleur_peer = 0;
